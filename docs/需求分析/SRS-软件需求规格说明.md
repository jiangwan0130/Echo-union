...
- **用户管理模块**：用户注册（管理员批量导入 + 学号默认密码）、登录、角色权限管理
...
| 需求名称 | 用户注册 |
| 优先级 | P0 |
| 描述 | 系统仅支持**管理员批量导入**方式完成注册/激活；不提供自助注册入口；~~邀请链接注册已废弃~~ |
...
- 方式A：管理员批量导入
  ...
  - 所属组织（默认下拉；预留后续可修改）
  - 所属部门（必填，下拉选择）
- ~~方式B：邀请链接注册/激活~~（**已废弃**，改为管理员批量导入时设置默认密码，用户首次登录后强制修改）

**处理逻辑**：
1. 管理员导入：校验必填字段、学号唯一性、邮箱格式与唯一性；创建用户记录并设置默认角色为"值班成员"；密码默认为学号后6位（首次登录需强制修改）；可按配置发送激活/通知邮件
2. ~~邀请链接~~（**已废弃**）

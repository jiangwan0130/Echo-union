...
| 功能域 | 说明 | 状态 |
|--------|------|------|
| 用户管理 | 邀请注册、角色权限、部门管理 | 一期 ✅ 已完成 |
| 时间收集 | ICS导入（文件/链接）、日历标记不可用时间 | 一期 ✅ 已完成 |
| 自动排班 | 约束规则配置、自动生成、手动调整、发布变更 | 一期 ✅ 已完成 |
| 换班管理 | 申请、同意、审核三步流程 | **二期** 🔜 |
| 签到签退 | Web签到/签退、缺席检测、补签 | **二期** 🔜 |
| 通知提醒 | 待办事项计算型接口（一期轻量）；邮件推送、偏好配置（**二期** 🔜） | 一期（部分）|
| 系统配置 | 学期管理、时间段配置、规则开关 | 一期 ✅ 已完成 |
| 数据导出 | 排班表Excel导出（一期）；签到统计导出（二期） | 一期/二期 |
...
> **事务保护策略：** 以下关键操作均使用 `BeginTx` 数据库事务保护，保证原子性：
> - **Register**（用户注册）：创建用户 + 创建学期分配 + 消费邀请码
> - **ImportICS**（课表导入）：清除旧课表 + 批量插入新课表
> - **不可用时间 CRUD**：创建/更新/删除不可用时间均在事务内完成
> - **Semester.Activate**（学期激活）：取消旧活动学期 + 激活新学期 + 批量创建用户分配
...
| 模块 | 职责 | 核心功能 | 状态 |
|------|------|----------|------|
| **Auth** | 认证授权 | 登录、JWT签发/刷新/验证、邀请链接生成与验证 | 一期 ✅ 已完成 |
| **User** | 用户管理 | 用户CRUD、角色分配、密码管理、批量导入 | 一期 ✅ 已完成 |
| **Department** | 部门管理 | 部门CRUD、成员关联、“需要值班”勾选 | 一期 ✅ 已完成 |
| **TimeTable** | 时间收集 | ICS解析、不可用时间CRUD、提交状态管理 | 一期 ✅ 已完成 |
| **Schedule** | 排班核心 | 自动排班算法、手动调整、发布、变更记录 | 一期 ✅ 已完成 |
| **Swap** | 换班管理 | 换班申请、冲突校验、审批流程、状态流转 | **二期** 🔜 |
| **Duty** | 签到签退 | 签到/签退、时间窗口校验、缺席检测、补签 | **二期** 🔜 |
| **Notify** | 通知服务 | **一期（轻量）**：`GET /notifications/pending` 基于 Semester.phase 实时计算待办列表，无独立通知表；**二期** 🔜：邮件推送、站内消息、通知偏好管理 | 一期/二期 |
| **Config** | 系统配置 | 学期管理、时间段配置、规则开关、系统参数 | 一期 ✅ 已完成 |
| **Export** | 数据导出 | 排班表Excel导出（一期 ✅ 已完成）、签到统计Excel导出（**二期** 🔜） | 一期/二期 |
...
│    • 值班成员列表（需要值班 + 已提交时间表）                                      │
│    • 成员时间表（课表 + 不可用时间）                                             │
│    • 不可用时间支持 once（单次）、weekly（每周）、biweekly（双周）三种 repeat_type  │
│    • 值班时间段配置（周一~四、周五）                                             │
│    • 排班规则配置（启用/禁用状态）                                               │
│    • 上学期值班记录（用于优化均衡）                                              │
│                                                                                 │
...
| v1.1 | 2026-02-26 | 同步代码实现进度：<br>1. 一期模块标记为“已完成”（Auth/User/Department/TimeTable/Schedule/Config/Export）<br>2. 不可用时间新增 biweekly（双周）repeat_type 支持<br>3. 补充事务保护策略说明（BeginTx） | 文档同步助手 |

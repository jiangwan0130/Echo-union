...
| 功能域 | 说明 | 阶段 |
|--------|------|------|
| 用户管理 | 邀请注册、角色权限、部门管理 | 一期 ✅ |
| 时间收集 | ICS导入（文件/链接）、日历标记不可用时间 | 一期 ✅ |
| 自动排班 | 约束规则配置、自动生成、手动调整、发布变更 | 一期 ✅ |
| 换班管理 | 申请、同意、审核三步流程 | **二期** 🔜 |
| 签到签退 | Web签到/签退、缺席检测、补签 | **二期** 🔜 |
| 通知提醒 | 邮件通知、站内消息、通知偏好配置 | **二期** 🔜 |
| 系统配置 | 学期管理、时间段配置、规则开关 | 一期 ✅ |
...
| 模块 | 职责 | 核心功能 | 阶段 |
|------|------|----------|------|
| **Auth** | 认证授权 | 登录、JWT签发/刷新/验证、邀请链接生成与验证 | 一期 ✅ |
| **User** | 用户管理 | 用户CRUD、角色分配、密码管理、批量导入 | 一期 ✅ |
| **Department** | 部门管理 | 部门CRUD、成员关联、"需要值班"勾选 | 一期 ✅ |
| **TimeTable** | 时间收集 | ICS解析、不可用时间CRUD、提交状态管理 | 一期 ✅ |
| **Schedule** | 排班核心 | 自动排班算法、手动调整、发布、变更记录 | 一期 ✅ |
| **Swap** | 换班管理 | 换班申请、冲突校验、审批流程、状态流转 | **二期** 🔜 |
| **Duty** | 签到签退 | 签到/签退、时间窗口校验、缺席检测、补签 | **二期** 🔜 |
| **Notify** | 通知服务 | 邮件发送、站内消息、通知偏好管理 | **二期** 🔜 |
| **Config** | 系统配置 | 学期管理、时间段配置、规则开关、系统参数 | 一期 ✅ |
| **Export** | 数据导出 | 排班表Excel导出（一期 ✅）、签到统计Excel导出（**二期** 🔜） | 一期/二期 |

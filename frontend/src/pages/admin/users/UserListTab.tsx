...
export default function UserListTab() {
  ...
  const handleCreateSubmit = async () => {
    let values: CreateUserRequest;
    try {
      values = await createForm.validateFields();
    } catch {
      // 表单校验未通过，antd 会在字段下方显示内联错误，不额外弹 toast
      return;
    }
  ...
  return (
    <div>
      ...
      <Flex justify="flex-end" style={{ marginBottom: 16 }}>
        <Space>
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={() => { setCreateResult(null); setCreateDrawerOpen(true); }}
          >
            新增用户
          </Button>
      ...
      <Drawer
        ...
        onClose={() => { setCreateDrawerOpen(false); setCreateResult(null); }}
        width={420}
        extra={
          !createResult && (
            <Space>
        ...
        {!createResult ? (
          ...
          <div>
            <Alert
              type="success"
              showIcon
              message={`用户 ${createResult.user.name} 创建成功`}
              description={
                <Flex align="center" gap={8} style={{ marginTop: 4 }}>
                  <Text>初始密码：</Text>
                  <Text code style={{ fontSize: 15 }}>{createResult.temp_password}</Text>
                  <Button
                    type="link"
                    icon={<CopyOutlined />}
                    onClick={() => copyToClipboard(createResult!.temp_password)}
                  >
                    复制
                  </Button>
                </Flex>
            ...
            />
            <Text type="secondary">用户首次登录后需修改密码。</Text>
            <div style={{ marginTop: 16 }}>
              <Button type="primary" onClick={() => setCreateResult(null)}>
                继续新增
              </Button>
              <Button style={{ marginLeft: 8 }} onClick={() => setCreateDrawerOpen(false)}>
                关闭
              </Button>
            </div>
          </div>
